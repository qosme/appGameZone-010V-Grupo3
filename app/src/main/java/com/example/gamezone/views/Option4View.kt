...
import com.example.gamezone.data.Preferencias
import com.example.gamezone.viewModels.Option4ViewModel
...
fun Option4View(
    ...
    LaunchedEffect(Unit) {
        Preferencias.suscripcionFlow(context).collect { saved ->
            if (ui.suscripcion != saved) vm.setSuscripcion(saved)
        }
    }
    // Guardar cada cambio
    LaunchedEffect(ui.suscripcion) {
        Preferencias.setSuscripcion(context, ui.suscripcion)
    }
